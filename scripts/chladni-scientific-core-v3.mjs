...
export function normalizeChladniModePair(n, m) {
  ...
  if (safeN === safeM) safeM += 1;
  if (safeM > 22) safeM = 22;
  if (safeN === safeM) safeN = Math.max(1, safeN - 1);
  if (safeM <= safeN) safeM = Math.min(22, safeN + 1);
  return { n: safeN, m: safeM };
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${svgEscape(title)}">
  <defs>
    ...
    <filter id="lineGlow" x="-25%" y="-25%" width="150%" height="150%">
      <feGaussianBlur stdDeviation="${glow ? activePalette.glowStrength : 0}" result="blur"/>
...
export function scientificRarityTier(seed, mode) {
  ...
  if (roll < 800) return 5;
  if (roll < 1800) return 4;
  if (roll < 3800) return 3;
  if (roll < 6300) return 2;
  if (roll < 8800) return 1;
  return 0;
}
