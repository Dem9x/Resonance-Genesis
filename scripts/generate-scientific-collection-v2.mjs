...
import {
  ...
  writeJson,
} from "./chladni-scientific-core-v2.mjs";

const ROOT = process.cwd();
const DEFAULT_OUT_DIR = path.join(ROOT, "output_scientific_collection_v2", "chladni-nodes");

function parseArgs(argv) {
  const args = { count: 1000, start: 1, seed: "resonance-genesis-scientific-v2", outDir: DEFAULT_OUT_DIR, imageBase: "ipfs://SCIENTIFIC_IMAGE_CID", imageExt: "svg", animationBase: "", animationExt: "mp4", includeAnimationUrl: false, width: 1024, height: 1024, resolution: 420 };
...
function printHelp() { console.log(`Generate scientific Chladni Node NFT collection v2.\nnode scripts/generate-scientific-collection-v2.mjs --count 1000`); }
...
function makeMetadata({ tokenId, mode, seed, imageUri, animationUri }) {
  ...
  const metadata = { name: `Chladni Node #${tokenId}`, description: "A scientific Chladni Node from the Resonance Genesis collection. Generated from deterministic plate resonance math, nodal line detection, and on-chain mining traits.", image: imageUri, tokenId, frequency: mode.frequency, waveNumberK: mode.waveNumberK, modeN: mode.n, modeM: mode.m, patternName: mode.patternName, family: mode.family, nodeDensity: Number((nodeDensityBps / 10000).toFixed(4)), nodeDensityBps, lineThickness: Number((lineThicknessBps / 100).toFixed(2)), lineThicknessBps, rarityTier, rarityName, frequencyBand: band, symmetryType: symmetry, nodeArchitecture: architecture, waveDistortion: distortion, harmonicLayerCount: harmonicLayers, hashratePreview, renderer: "scientific-chladni-node-v2", attributes: [{ trait_type: "Frequency", value: `${mode.frequency} Hz` }, { trait_type: "Wave Number K", value: mode.waveNumberK }, { trait_type: "Frequency Band", value: band }, { trait_type: "Mode", value: `${mode.n} x ${mode.m}` }, { trait_type: "Mode N", value: mode.n }, { trait_type: "Mode M", value: mode.m }, { trait_type: "Pattern", value: mode.patternName }, { trait_type: "Family", value: mode.family }, { trait_type: "Node Density", value: Number((nodeDensityBps / 10000).toFixed(4)) }, { trait_type: "Node Density BPS", value: nodeDensityBps }, { trait_type: "Line Thickness", value: Number((lineThicknessBps / 100).toFixed(2)) }, { trait_type: "Line Thickness BPS", value: lineThicknessBps }, { trait_type: "Rarity Tier", value: rarityName }, { trait_type: "Rarity Index", value: rarityTier }, { trait_type: "Symmetry Type", value: symmetry }, { trait_type: "Node Architecture", value: architecture }, { trait_type: "Wave Distortion", value: distortion }, { trait_type: "Harmonic Layer Count", value: harmonicLayers }, { trait_type: "Hashrate Preview", value: hashratePreview }, { trait_type: "Visual Style", value: "Scientific Chladni BW" }] };
  if (animationUri) metadata.animation_url = animationUri;
  return { metadata, onchainTrait: { frequency: mode.frequency, modeN: mode.n, modeM: mode.m, nodeDensityBps, lineThicknessBps, rarityTier, initialized: true }, summaryTraits: { rarityName, band, symmetry, architecture, distortion, harmonicLayers, hashratePreview } };
}
...
function main() {
  ...
  ensureOutput(outDir);
  const summary = { name: "Resonance Genesis", collection: "Chladni Node", renderer: "scientific-chladni-node-v2", total: options.count, startTokenId: options.start, endTokenId: options.start + options.count - 1, frequencyRange: { min: SCI_MIN_FREQUENCY, max: SCI_MAX_FREQUENCY }, generatedAt: new Date().toISOString(), seed: options.seed, imageBase: options.imageBase, animationBase: options.animationBase || null, frequencies: {}, waveNumbers: {}, families: {}, rarityTiers: {}, frequencyBands: {}, symmetryTypes: {}, nodeArchitectures: {}, waveDistortions: {}, harmonicLayerCounts: {}, minHashratePreview: null, maxHashratePreview: null };
  const onchainTraits = { tokenIds: [], traits: [] };
  const csvRows = [["tokenId", "name", "frequency", "waveNumberK", "modeN", "modeM", "nodeDensityBps", "lineThicknessBps", "rarityTier", "rarityName", "family", "patternName", "frequencyBand", "symmetryType", "nodeArchitecture", "waveDistortion", "harmonicLayerCount", "hashratePreview", "image", "animation_url"]];
  console.log("Generating Resonance Genesis scientific Chladni collection v2...");
  for (let i = 0; i < options.count; i += 1) {
    ...
    for (const [obj, key] of [[summary.frequencies, String(mode.frequency)], [summary.waveNumbers, String(mode.waveNumberK)], [summary.families, mode.family], [summary.rarityTiers, summaryTraits.rarityName], [summary.frequencyBands, summaryTraits.band], [summary.symmetryTypes, summaryTraits.symmetry], [summary.nodeArchitectures, summaryTraits.architecture], [summary.waveDistortions, summaryTraits.distortion], [summary.harmonicLayerCounts, String(summaryTraits.harmonicLayers)]]) incrementCounter(obj, key);
